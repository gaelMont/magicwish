...
export default function CollectionPage() {
  ...
  const filteredAndSortedCards = useMemo(() => {
    ...
    if (filterTrade) result = result.filter(c => c.isForTrade);
  ...
  if (loading) return <p className="text-center p-10 text-gray-500">Chargement...</p>;
  if (!user) return <p className="text-center p-10">Veuillez vous connecter.</p>;

  return (
    <main className="container mx-auto p-4 pb-28 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-2">
           ...
           {!isSelectMode && (
               <>
                <button onClick={() => setIsToolsOpen(true)} className="bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-200 px-3 py-2 rounded-lg text-sm font-medium border border-gray-200 dark:border-gray-600">⚙️ Gérer</button>
                <DeleteAllButton targetCollection="collection" />
                <button onClick={() => setIsImportOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm">Importer CSV</button>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6 space-y-4 md:space-y-0 md:flex md:items-end md:gap-4">
          <div className="flex-grow">
              ...
              <input type="text" placeholder="Rechercher..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-sm outline-none" />
          </div>
          <div className="min-w-[200px]">
              <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Édition</label>
              <select value={filterSet} onChange={(e) => setFilterSet(e.target.value)} className="w-full p-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-sm outline-none cursor-pointer">
          ...
          <div className="min-w-[180px]">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="w-full p-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-sm outline-none cursor-pointer">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredAndSortedCards.map((card) => (
            <MagicCard 
              ...
              onDelete={() => setCardToDelete(card.id)} // (Optionnel si pas de bouton UI)
      ...
      {isSelectMode && selectedIds.length > 0 && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 shadow-2xl border border-gray-200 dark:border-gray-700 p-2 rounded-2xl flex items-center gap-2 z-50 animate-in slide-in-from-bottom-6 duration-300">
              ...
              >
                  <span>🤝</span>
                  <span className="text-[10px]">Mettre Trade</span>
              </button>
              <button 
              ...
              >
                  <span>🔒</span>
                  <span className="text-[10px]">Enlever Trade</span>
              </button>
              
              <div className="w-px h-8 bg-gray-300 mx-1"></div>

              {/* LE BOUTON SUPPRIMER SÉLECTION EST ICI */}
              <button 
      ...
      <ConfirmModal isOpen={!!cardToDelete} onClose={() => setCardToDelete(null)} onConfirm={() => { if(cardToDelete) removeCard(cardToDelete); }} />
    </main>
  );
}