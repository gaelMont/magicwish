  ...
  return (
    <div className="container mx-auto p-4 h-[calc(100vh-64px)] flex flex-col">
        <div className="flex-none flex items-center gap-4 mb-4">
            <button onClick={() => router.back()} className="text-gray-500 hover:text-gray-700 bg-gray-100 px-3 py-1 rounded-lg text-sm">← Retour</button>
            <h1 className="text-2xl font-bold truncate">Échange avec <span className="text-blue-600">{targetName}</span></h1>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 grow overflow-hidden pb-24">
            <div className="flex flex-col h-full bg-red-50/50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900 overflow-hidden relative shadow-sm">
                <div className="p-4 pb-0 flex-none">
                     ...
                     <input type="text" placeholder="Filtrer ma collection..." className="w-full p-2 mb-2 rounded border dark:bg-gray-800 dark:text-white dark:border-gray-600 text-sm" value={searchMe} onChange={e => setSearchMe(e.target.value)} />
                </div>
                <TradeSelectionTable cards={toGive} onRemove={(id) => handleRemoveCard(id, 'give')} onUpdatePrice={(id, p) => handleUpdatePrice(id, p, 'give')} colorClass="text-danger" emptyLabel="Sélectionnez vos cartes..." />
            ...
            <div className="flex flex-col h-full bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900 overflow-hidden relative shadow-sm">
                <div className="p-4 pb-0 flex-none">
                    ...
                    <input type="text" placeholder={`Filtrer chez ${targetName}...`} className="w-full p-2 mb-2 rounded border dark:bg-gray-800 dark:text-white dark:border-gray-600 text-sm" value={searchHim} onChange={e => setSearchHim(e.target.value)} />
                </div>
                <TradeSelectionTable cards={toReceive} onRemove={(id) => handleRemoveCard(id, 'receive')} onUpdatePrice={(id, p) => handleUpdatePrice(id, p, 'receive')} colorClass="text-success" emptyLabel="Sélectionnez les cartes de l'ami..." />
                ...
                <TradeSourceTable cards={filteredFriendCollection} onAdd={(c) => handleSelectCard(c, 'receive')} buttonColorClass="text-blue-600" loading={loadingHim} />
            </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 h-20 bg-white dark:bg-gray-900 border-t dark:border-gray-800 flex items-center px-6 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
                ...
                <div className={`text-2xl font-black ${balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>{balance > 0 ? '+' : ''}{balance.toFixed(2)} €</div>
            </div>
            <div className="flex-1 flex justify-end">
                <button onClick={handlePropose} disabled={toGive.length === 0 && toReceive.length === 0} className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-3 rounded-xl font-bold disabled:opacity-50 transition shadow-lg transform active:scale-95">Proposer</button>
            </div>
        </div>
    </div>
  );
